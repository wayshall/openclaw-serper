...
- If found there, pass the env vars when running the script via `exec`:
  ```bash
  SERPER_API_KEY="xxx" SERP_SEARCH_URL="..." SERP_NEWS_URL="..." python3 scripts/search.py ...
  ```
- Only suggest setup if the key is truly missing from all three sources
